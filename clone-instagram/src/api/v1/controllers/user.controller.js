...
exports.signup = async(req, res) => {
    try {
        ...
        if (!passwordValidator(password))
            return res.status(400).send({
                message: "NOTE: Weak password so, therefore, try a combination of numbers/symbols and a good length",
            });
        ...
        if (user) {
            if (user.email === email && user.verifiedUser === true) {
                return res.status(400).send({
                    message: "The email already exists so try forgot password!",
                });
            } else if (user.email === email) {
                user = await UserService.findOneAndUpdate({ email }, {
                    username,
                    email,
                    name,
                    ...rest,
        ...
        return res.status(201).send({
            message: "Check your email. We have sent you a mail.",
        });
    } catch (e) {
